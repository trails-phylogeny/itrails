...
def trans_emiss_calc(t_A, t_B, t_C, t_2, t_upper, t_out, N_AB, N_ABC, r, n_int_AB, n_int_ABC,
    ...
    tr = pd.DataFrame.from_dict(tr_dict, orient='index', columns=['Prob'])
    tr.index = pd.MultiIndex.from_tuples(tr.index, names=['From', 'To'])

    # Pivot to create the transition matrix
    tr = tr.unstack(fill_value=0)
