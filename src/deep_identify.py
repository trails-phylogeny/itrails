...
# Define Numba types
tuple_key_type = types.UniTuple(int64, 2)  # (int64, int64)
path_type = types.ListType(tuple_key_type)  # List of (int64, int64)
paths_list_type = types.ListType(
    path_type
...
def generate_paths_deep(
    ...
    if diff_l <= 1 and diff_r <= 1:
        ...
        if key not in all_paths_dict:
            all_paths_dict[key] = List.empty_list(path_type)
        # Create a copy of the current path
        path_copy = List.empty_list(tuple_key_type)
        for p in path:
            path_copy.append(p)
...
def get_all_paths_deep(
    ...
    all_paths_dict = Dict.empty(key_type=key_type, value_type=paths_list_type)
    path = List.empty_list(tuple_key_type)
...
omega_nonrev_counts = Dict.empty(key_type=int64, value_type=int64)
inverted_omega_nonrev_counts = Dict.empty(
    key_type=int64, value_type=types.ListType(int64)
)
...
inverted_omega_nonrev_counts[0] = List.empty_list(int64)
inverted_omega_nonrev_counts[0].append(0)

inverted_omega_nonrev_counts[1] = List.empty_list(int64)
inverted_omega_nonrev_counts[1].append(3)
inverted_omega_nonrev_counts[1].append(5)
inverted_omega_nonrev_counts[1].append(6)

inverted_omega_nonrev_counts[2] = List.empty_list(int64)
